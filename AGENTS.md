...
- **Purpose:** Facial recognition microservice for the [Lychee](https://github.com/LycheeOrg/Lychee) photo gallery
...
2. Read [`docs/1-concepts/README.md`](docs/1-concepts/README.md) — domain model (embeddings, detection pipeline, clustering, callbacks).
...
- `<feature-name>` is 2–4 hyphen-separated lowercase words in action-noun format (e.g. `add-blur-filter`, `fix-callback-retry`).
...
- CPU-bound work (DeepFace) must be offloaded via `loop.run_in_executor(executor, ...)`.
- Routes are thin — delegate to domain modules in `app/detection/`, `app/clustering/`, etc.
